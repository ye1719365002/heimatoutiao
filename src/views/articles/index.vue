<template>
    <el-card class="articles">
        ...
        <div class='article-item' v-for="item in list" :key="item.id.toString()">
            <!-- 左侧 -->
            <div class="left">
                <img :src="item.cover.images.length? item.cover.image[0]:defaultImg " alt="">
                <div class="info">
                    <span>{{item.title}} </span>
                    <!-- 文章状态 0-草稿，1-待审核，2-审核通过，3-审核失败，4-已删除 -->
                    <el-tag :type="item.stasus | filterType" >{{item.status| filterStatus }} </el-tag>
...
export default {
  data () {
    return {
      ...
      channels: [], // 接收频道数据
      list: [],
      defaultImg: require('../../assets/img/default.gif') // 默认图片
    }
  },
  filters: {
    //   过滤器第一个参数 是value
